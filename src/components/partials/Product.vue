<template>
  <div class="product">
    <figure class="product-media">
      ...
      <router-link :to="{ name: 'stores.products.show', params: { id: item.id, store: item.store.slug } }">
      ...
      <div class="product-action">
        <a @click.prevent="$cart.addItem(item.store, item)" href="#" class="btn-product btn-cart" title="Add to cart"><span>add to cart</span></a>
      </div><!-- End .product-action -->
    </figure><!-- End .product-media -->

    <div class="product-body">
      <div class="product-cat">
        <router-link v-if="item.category.matchCategory" :to="{ name: 'categories.show', params: { id: item.category.matchCategory.id } }">{{ item.category.matchCategory.name }}</router-link>
        <router-link v-else :to="{ name: 'stores.categories.show', params: { id: item.category.id, store: item.store.slug } }">{{ item.category.name }}</router-link>
      </div><!-- End .product-cat -->
      <h3 class="product-title"><router-link :to="{ name: 'stores.products.show', params: { id: item.id, store: item.store.slug } }">{{ item.title }}</router-link></h3><!-- End .product-title -->
